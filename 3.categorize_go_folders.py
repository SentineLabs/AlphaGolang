...
def folder_sort_and_nest():
    common_packages = ["archive", "bufio", "builtin", "bytes", "compress", "container", "context", "crypto", "database", "debug", "embed", "encoding", "errors", "flag", "fmt", "go", "hash", "html", "image", "index", "io", "log", "math", "mime", "net", "os", "path", "plugin", "regexp", "sort", "strconv", "strings", "sync", "syscall", "testing", "text", "time", "unicode", "unsafe", "internal", "reflect", "vendor", "golang", "runtime", "type", "setg", "pthread", "walk"]
...
def handle_unnested():
    unnested = {"uncategorized", "unnamed", "nullsub"}
    for key in unnested:
        if folders.get(key) is None:
            continue
        print(folders[key]) #DEBUG
        for name in folders[key]:
            try:
                func_dir.rename(name, key+"/"+name)
            except:
                print("Failed to move function:", name)
